...
logging.getLogger("TEST_TRT").setLevel(logging.INFO)
log = logging.getLogger("TEST_TRT")
...
def main():
    ...
    # Make Trt runtime Session
    with open(args.input, "rb") as f:
        engine = runtime.deserialize_cuda_engine(f.read())
    context = engine.create_execution_context()

    inputs, input_shape, input_type, outputs, output_shapes, bindings, stream, batch_size = allocate_buffers(engine)
    log.info(f"{input_shape}, {input_type}, {output_shapes}, {stream}")

    log.info(f"** Input Tensor shape / type: {input_shape}, {input_type}")
    for _shape in output_shapes:
        log.info(f"** Output Tensors shape : {_shape}")
    ...
    if faces is not None:
        ...
        filename = f'{os.path.splitext(args.input)[0]}.jpg'
...
def allocate_buffers(engine):
    ...
    return inputs, input_shape, input_type, outputs, output_shapes, bindings, stream, batch_size


def preprocess(input, img):
    scales = [640, 640]
    img, im_scale, add_w, add_h = resize_image(img, scales)
    ...
    img = cp.transpose(im, [2, 0, 1])
    img = cp.ascontiguousarray(img)
    img = cp.expand_dims(img, axis=0)

    cp.copyto(input[0], img)

    return img, im.shape[0:2], im_scale
...
def do_inference(stream, context, bindings):
    with stream:
